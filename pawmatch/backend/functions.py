...
def connect_to_db():
    return mysql.connector.connect(
        host="localhost",
        user="root",
        password="Guarita12!",  # Replace with your MySQL root password
...
def insert_dog(staff_id, name, breed, age, arrival_date, spayed_neutered, adoptability_score, sex, initial_status, kennel_no=None, date_start_availability=None, main_image_url=None, extra_image_urls=None):
    ...
    # Insert dog record
    add_dog = """
    INSERT INTO Dog (name, breed, age, adoptabilityScore, arrivalDate, spayedNeuturedStatus, sex) 
...
def delete_dog(dog_id):
    ...
    try:
        ...
        db.rollback()
        print(f"Failed to delete dog ID {dog_id}: {error}")
...
def print_medical_history(dog_id):
    ...
    cursor.execute("SELECT spayedNeuturedStatus FROM Dog WHERE dogID = %s", (dog_id,))
...
def main():
    ...
    while True:
        ...
        print("4. Delete Dog")
        print("5. Delete Medical Procedure")
        print("6. Delete Vaccine")
        ...
        if choice == '1':
            ...
            dog_id = int(input("Enter dog ID of dog toy want to delete: "))
            delete_dog(dog_id)
